...
public class DBAppTestsMS2 {
	...
	public void test0TableValidationSoundDBApp() throws Exception {
		...
		assertTrue("Validating a non-afected table should report zero sized missing records list.",
				tr_rrkg.size() == 0);
		// --------------------------------------------------------------------------
	...
	public void test0TableValidationSoundTraceDBApp() throws Exception {
		...
		assertTrue("Validating a non-afected table should report zero missing records.",
				tableTrace0.contains("0 records missing."));
		// --------------------------------------------------------------------------
	...
	public void test0TableValidationAffectedDBApp() throws Exception {
		...
		for (int i = 0; i < pageCount0 - 1; i++) {
			if (Math.random() > 0.75) {
				File dir_y633 = new File(
						FileManager.directory.getAbsolutePath() + File.separator + "y633" + File.separator + i + ".db");
				dir_y633.delete();
				randDel0++;
			}
		}
		ArrayList<String[]> tableValidate0 = DBApp.validateRecords("y633");
		assertTrue("Validating a table with some pages deleted should report the correct size of missing records list.",
				tableValidate0.size() == (randDel0 * DBApp.dataPageSize));
		// --------------------------------------------------------------------------
	...
	public void test0TableValidationAffectedTraceDBApp() throws Exception {
		...
		for (int i = 0; i < pageCount0 - 1; i++) {
			if (Math.random() > 0.75) {
				File dir_j4 = new File(
						FileManager.directory.getAbsolutePath() + File.separator + "j4" + File.separator + i + ".db");
				dir_j4.delete();
				randDel0++;
			}
		}
		...
		assertTrue("Validating a table should appear in the last trace.", tableTrace0.contains("Validating records"));
		assertTrue(
				"Validating a Validating a table with some pages deleted should report the number of missing records.",
				tableTrace0.contains((randDel0 * DBApp.dataPageSize) + " records missing."));
		// --------------------------------------------------------------------------
	...
	public void test0TableRecoverDBApp() throws Exception {
		...
		// q9]
		// record452: [a452, b0, c2, d0, e2, f2, g4, h4, i2, j2, k1, l8, m10, n4, o2,
		// p4, q10]
		// record453: [a453, b1, c0, d1, e3, f3, g5, h5, i3, j3, k2, l9, m11, n5, o3,
		// p5, q11]
		// record454: [a454, b0, c1, d2, e4, f4, g6, h6, i4, j4, k3, l10, m12, n6, o4,
		// p6, q12]
		// record455: [a455, b1, c2, d3, e0, f5, g0, h7, i5, j5, k4, l11, m0, n7, o5,
		// p7, q13]
		...
		for (int i = 0; i < pageCount0 - 1; i++) {
			if (Math.random() > 0.75) {
				File dir_z21 = new File(
						FileManager.directory.getAbsolutePath() + File.separator + "z21" + File.separator + i + ".db");
				dir_z21.delete();
		...
		assertTrue("The sizes of select lists before and after recovery should be of equal sizes.",
				selectBeforeRecovery0.size() == selectAfterRecovery0.size());
		for (int i = 0; i < selectBeforeRecovery0.size(); i++) {
			assertTrue("The recovery process should result in returning records to their original location.",
					Arrays.toString(selectBeforeRecovery0.get(i)).equals(Arrays.toString(selectAfterRecovery0.get(i))));
		}
		// --------------------------------------------------------------------------
	...
	public void test0TableRecoverTrceDBApp() throws Exception {
		...
		for (int i = 0; i < pageCount0 - 1; i++) {
			if (Math.random() > 0.75) {
				File dir_z0g = new File(
						FileManager.directory.getAbsolutePath() + File.separator + "z0g" + File.separator + i + ".db");
				dir_z0g.delete();
				deletedPages0.add(i);
				for (int j = i * DBApp.dataPageSize; j < i * DBApp.dataPageSize + DBApp.dataPageSize; j++) {
		...
		assertTrue("Recovering missing records from a table should appear in the last trace.",
				tableTrace0.contains("Recovering"));
		assertTrue("Recovering missing records from a table should report the correct number of recovered records.",
				tableTrace0.contains(missing0.size() + " records"));
		assertTrue(
				"Recovering missing records from a table should report the correct pages list in which recovery took effect.",
				tableTrace0.contains("in pages: " + deletedPages0));
		// --------------------------------------------------------------------------
	...
	public void test0TableSelectionIndexConditionNonIndexedDBApp() throws Exception {
		...
		assertTrue(
				"Conditional Index selection when no columns are indexed should return the correct number of records.",
				tableSelect0.size() == 46);
		...
		assertTrue("Conditional Index selection when no columns are indexed should return the correct inserted record.",
				tableSelectStrings0.contains(Arrays.toString(new String[] { "a3", "b1", "c0", "d3", "e3", "f3", "g3",
						"h3", "i3", "j3", "k3", "l3", "m3", "n3", "o3" })));
		assertTrue("Conditional Index selection when no columns are indexed should return the correct inserted record.",
				tableSelectStrings0.contains(Arrays.toString(new String[] { "a8", "b0", "c2", "d0", "e3", "f2", "g1",
						"h0", "i8", "j8", "k8", "l8", "m8", "n8", "o8" })));
		assertTrue("Conditional Index selection when no columns are indexed should return the correct inserted record.",
				tableSelectStrings0.contains(Arrays.toString(new String[] { "a13", "b1", "c1", "d1", "e3", "f1", "g6",
						"h5", "i4", "j3", "k2", "l1", "m0", "n13", "o13" })));
		assertTrue("Conditional Index selection when no columns are indexed should return the correct inserted record.",
				tableSelectStrings0.contains(Arrays.toString(new String[] { "a18", "b0", "c0", "d2", "e3", "f0", "g4",
						"h2", "i0", "j8", "k7", "l6", "m5", "n4", "o3" })));
		assertTrue("Conditional Index selection when no columns are indexed should return the correct inserted record.",
				tableSelectStrings0.contains(Arrays.toString(new String[] { "a23", "b1", "c2", "d3", "e3", "f5", "g2",
						"h7", "i5", "j3", "k1", "l11", "m10", "n9", "o8" })));
		// --------------------------------------------------------------------------
	...
	public void test0TableSelectionIndexConditionNonIndexedTraceDBApp() throws Exception {
		...
		assertTrue("Select Index when no columns are indexed Should appear in the last trace.",
				tableTrace0.contains("Select index"));
		assertTrue("Select Index when no columns are indexed Should Not report indexed columns when non are.",
				!tableTrace0.contains("Indexed columns:"));
		assertTrue("Select Index when no columns are indexed Should report non indexed columns.",
				tableTrace0.contains("Non Indexed:"));
		assertTrue("Select Index when no columns are indexed Should report the correct set of non indexed columns.",
				tableTrace0.contains(Arrays.toString(ConditionColumns0)));
		assertTrue("Select Index when no columns are indexed Should report the correct number of selected records.",
				tableTrace0.contains("Final count: " + 13));
		// --------------------------------------------------------------------------
	...
	public void test0TableSelectionIndexConditionSomeIndexedDBApp() throws Exception {
		...
		assertTrue(
				"Conditional Index selection when some columns are indexed should return the correct number of records.",
				tableSelect0.size() == 14);
		ArrayList<String> tableSelectStrings0 = new ArrayList<String>();
		for (int i = 0; i < tableSelect0.size(); i++)
			tableSelectStrings0.add(Arrays.toString(tableSelect0.get(i)));
		assertTrue(
				"Conditional Index selection when some columns are indexed should return the correct inserted record.",
				tableSelectStrings0
						.contains(Arrays.toString(new String[] { "a24", "b0", "c0", "d0", "e4", "f0", "g3" })));
		assertTrue(
				"Conditional Index selection when some columns are indexed should return the correct inserted record.",
				tableSelectStrings0
						.contains(Arrays.toString(new String[] { "a54", "b0", "c0", "d2", "e4", "f0", "g5" })));
		assertTrue(
				"Conditional Index selection when some columns are indexed should return the correct inserted record.",
				tableSelectStrings0
						.contains(Arrays.toString(new String[] { "a84", "b0", "c0", "d0", "e4", "f0", "g0" })));
		assertTrue(
				"Conditional Index selection when some columns are indexed should return the correct inserted record.",
				tableSelectStrings0
						.contains(Arrays.toString(new String[] { "a114", "b0", "c0", "d2", "e4", "f0", "g2" })));
		assertTrue(
				"Conditional Index selection when some columns are indexed should return the correct inserted record.",
				tableSelectStrings0
						.contains(Arrays.toString(new String[] { "a144", "b0", "c0", "d0", "e4", "f0", "g4" })));
		// --------------------------------------------------------------------------
	...
	public void test0TableSelectionIndexConditionSomeIndexedTraceDBApp() throws Exception {
		...
		assertTrue("Select Index when some columns are indexed Should appear in the last trace.",
				tableTrace0.contains("Select index"));
		assertTrue("Select Index when some columns are indexed Should report indexed columns.",
				tableTrace0.contains("Indexed columns:"));
		assertTrue("Select Index when some columns are indexed Should report the correct set of indexed columns.",
				tableTrace0.contains("[b, d, i]"));
		assertTrue("Select Index when some columns are indexed Should report non indexed columns.",
				tableTrace0.contains("Non Indexed:"));
		assertTrue("Select Index when some columns are indexed Should report the correct set of non indexed columns.",
				tableTrace0.contains(Arrays.toString(ConditionColumns0)));
		assertTrue("Select Index when some columns are indexed Should report the correct number of selected records.",
				tableTrace0.contains("Final count: " + 1));
		// --------------------------------------------------------------------------
	...
	public void test0TableSelectionIndexConditionAllIndexedDBApp() throws Exception {
		...
		assertTrue(
				"Conditional Index selection when all columns are indexed should return the correct number of records.",
				tableSelect0.size() == 28);
		ArrayList<String> tableSelectStrings0 = new ArrayList<String>();
		for (int i = 0; i < tableSelect0.size(); i++)
			tableSelectStrings0.add(Arrays.toString(tableSelect0.get(i)));
		assertTrue(
				"Conditional Index selection when all columns are indexed should return the correct inserted record.",
				tableSelectStrings0.contains(Arrays
						.toString(new String[] { "a6", "b0", "c0", "d2", "e1", "f0", "g6", "h6", "i6", "j6", "k6" })));
		assertTrue(
				"Conditional Index selection when all columns are indexed should return the correct inserted record.",
				tableSelectStrings0.contains(Arrays
						.toString(new String[] { "a18", "b0", "c0", "d2", "e3", "f0", "g4", "h2", "i0", "j8", "k7" })));
		assertTrue(
				"Conditional Index selection when all columns are indexed should return the correct inserted record.",
				tableSelectStrings0.contains(Arrays
						.toString(new String[] { "a30", "b0", "c0", "d2", "e0", "f0", "g2", "h6", "i3", "j0", "k8" })));
		assertTrue(
				"Conditional Index selection when all columns are indexed should return the correct inserted record.",
				tableSelectStrings0.contains(Arrays
						.toString(new String[] { "a42", "b0", "c0", "d2", "e2", "f0", "g0", "h2", "i6", "j2", "k9" })));
		assertTrue(
				"Conditional Index selection when all columns are indexed should return the correct inserted record.",
				tableSelectStrings0.contains(Arrays.toString(
						new String[] { "a54", "b0", "c0", "d2", "e4", "f0", "g5", "h6", "i0", "j4", "k10" })));
		// --------------------------------------------------------------------------
	...
	public void test0TableSelectionIndexConditionAllIndexedTraceDBApp() throws Exception {
		...
		assertTrue("Select Index when all columns are indexed Should appear in the last trace.",
				tableTrace0.contains("Select index"));
		assertTrue("Select Index when all columns are indexed Should report indexed columns.",
				tableTrace0.contains("Indexed columns:"));
		assertTrue("Select Index when all columns are indexed Should report the correct set of indexed columns.",
				tableTrace0.contains(Arrays.toString(ConditionColumns0)));
		assertTrue("Select Index when all columns are indexed Should not report non indexed columns.",
				!tableTrace0.contains("Non Indexed:"));
		assertTrue("Select Index when all columns are indexed Should report the correct number of selected records.",
				tableTrace0.contains("Final count: " + 1));
		// --------------------------------------------------------------------------
	...
	public void test0CreationIndexFullTraceDBApp() throws Exception {
		...
		assertTrue("Full trace should report empty list when reporting indexed columns and non are.",
				tableTrace0.endsWith("[]"));
		...
		assertTrue("Full trace should report the corect list when reporting indexed columns and all are.",
				tableTrace0.endsWith("[a, b, c, d, e, f, g, h, i, j, k, l, m]"));
		// --------------------------------------------------------------------------
	...
	public void test0TableInsertionPreIndexingDBApp() throws Exception {
		...
		assertTrue("Bitmap index should be calculated correctly for all values.",
				DBApp.getValueBits("rv", "c", "c0").equals("1001001001001001001001"));
		assertTrue("Bitmap index should be calculated correctly for all values.",
				DBApp.getValueBits("rv", "e", "e0").equals("1000010000100001000010"));
		assertTrue("Bitmap index should return a sequence of zeros fo non-present values.",
				DBApp.getValueBits("rv", "g", "g7").equals("0000000000000000000000"));
		// --------------------------------------------------------------------------
	...
	public void test0TableInsertionPostIndexingDBApp() throws Exception {
		...
		// record19: [a19, b1, c1, d3, e4, f1, g5]
		assertTrue("Bitmap index should be calculated correctly for all values.",
				DBApp.getValueBits("u2d", "b", "b0").equals("10101010101010101010"));
		assertTrue("Bitmap index should be calculated correctly for all values.",
				DBApp.getValueBits("u2d", "c", "c1").equals("01001001001001001001"));
		assertTrue("Bitmap index should be calculated correctly for all values.",
				DBApp.getValueBits("u2d", "d", "d0").equals("10001000100010001000"));
		assertTrue("Bitmap index should be calculated correctly for all values.",
				DBApp.getValueBits("u2d", "f", "f0").equals("10000010000010000010"));
		assertTrue("Bitmap index should return a sequence of zeros fo non-present values.",
				DBApp.getValueBits("u2d", "g", "g7").equals("00000000000000000000"));
		// --------------------------------------------------------------------------
